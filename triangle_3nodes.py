...
def B_and_Ke_elem(gauss_coord,gauss_weight,elem_coord,connectivity,
    ...
    """
    Function to calculate B_elem and K_e 
    ...
    #Ke Matrix    
    K_e=Ke_matrix(K_e,gauss_weight,n_gauss,det_Jacobian,
...
def Internal_force(gauss_weight,det_Jacobian,B_elem,B_Gauss,B_t,Fi_elem,
    ...
    for n_g in range(n_gauss):
        ...
        w=gauss_weight[n_g,0]
        A=det_Jacobian[n_g]*w
        Sigm_we=stress_gauss_elem[:,n_g]*A/2
        Sigm_we_collumn=Sigm_we.reshape(-1, 1)
        Fi_elem=Fi_elem+np.dot(B_t,Sigm_we_collumn)*thickness   
