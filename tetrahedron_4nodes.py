...
@jit(nopython=True,cache=True)
def get_Ke(gauss_weight,det_Jacobian,B_elem,B_t,B_Gauss,tang_modu,K_e,thickness):
...
@jit(nopython=True,cache=True)
def get_Be(gauss_coord,gauss_weight,elem_coord,connectivity,
           jacobian,det_Jacobian,deri_phi_param,deri_phi_real,B_elem,
...
def Interpol_fun_derivative(gauss_coord,elem_coord,jacobian,n_gauss,deri_phi_param,
    ...
    if mesh_type=='Abaqus':
        ...
        for n_g in range(n_gauss):
            ...
            #dx_dt
            jacobian[2,0]=-cd_no1[0]+cd_no4[0]
            #dy_dr
            jacobian[0,1]=-cd_no1[1]+cd_no2[1]
            #dy_ds
            jacobian[1,1]=-cd_no1[1]+cd_no3[1]
            #dy_dt       
            jacobian[2,1]=-cd_no1[1]+cd_no4[1]
            #dz_dr       
            jacobian[0,2]=-cd_no1[2]+cd_no2[2] 
            #dz_ds
            jacobian[1,2]=-cd_no1[2]+cd_no3[2]
            #dz_dt      
            jacobian[2,2]=-cd_no1[2]+cd_no4[2]
            ...
            deri_phi_param[0,0]=-1 #dphi_1/dr
            deri_phi_param[0,1]=1  #dphi_2/dr
            deri_phi_param[0,2]=0  #dphi_3/dr
            deri_phi_param[0,3]=0  #dphi_4/dr

            
            deri_phi_param[1,0]=-1  #dphi_1/ds
            deri_phi_param[1,1]=0   #dphi_2/ds
            deri_phi_param[1,2]=1   #dphi_3/ds
            deri_phi_param[1,3]=0   #dphi_4/ds

            
            deri_phi_param[2,0]=-1  #dphi_1/dt
            deri_phi_param[2,1]=0   #dphi_2/dt
            deri_phi_param[2,2]=0   #dphi_3/dt
            deri_phi_param[2,3]=1   #dphi_4/dt
...
@jit(nopython=True,cache=True)
def get_det_Jacobian(gauss_coord,elem_coord,jacobian,n_gauss,det_Jacobian,mesh_type):
    cd_no1=elem_coord[0,:] 
    cd_no2=elem_coord[1,:]  
    cd_no3=elem_coord[2,:]  
    cd_no4=elem_coord[3,:] 
...
def Ke_matrix(gauss_weight,n_gauss,det_Jacobian,B_elem,B_Gauss,B_t,tang_modu,K_e):
    ...
    """    
    #TRANSFORMAR TANG_MODULU PARA ENTRAR COM O VALOR DE TODOS OS PONTOS DE GAUSS DO ELEMENTO
...
@jit(nopython=True,cache=True)
def Internal_force(gauss_weight,det_Jacobian,B_elem,B_Gauss,B_t,Fi_elem,
                   stress_gauss_elem,gauss_coord,elem_coord,jacobian,mesh_type,thickness):
    
    n_gauss=1
    gauss_weight[:,:]=1
    
    #Jacobian Determinant               
    # det_Jacobian=get_det_Jacobian(gauss_coord,elem_coord,jacobian,n_gauss,det_Jacobian,mesh_type)    
    ...
    for n_g in range(n_gauss):
        ...
        w=gauss_weight[n_g,0]         
        A=det_Jacobian[n_g]*w
        Sigm_we=stress_gauss_elem[:,n_g]*A*1/6
        Sigm_we_collumn=Sigm_we.reshape(-1, 1)
        Fi_elem+=np.dot(B_t,Sigm_we_collumn)                                                                                
        cont_g=cont_g+6        
    return Fi_elem
